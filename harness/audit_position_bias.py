...
run recorded:
    ...
    python -m harness.audit_position_bias
...
def build_report(results: Path) -> str:
    ...
    w("")
    w(f"Generated by `python -m harness.audit_position_bias` at {stamp_utc(now_utc())}, from the")
    ...
    w("per-item seeded RNG (`random.Random(f\"{seed}:{item_id}\")` in `harness/data.py`'s")
    w("`build_item`) and then held fixed across all five conditions -- `harness/repair.py` builds")
