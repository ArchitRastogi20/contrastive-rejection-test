...
files already in ``code/results/``.
    ...
    python -m harness.analyze_run3                    # writes the run-3 analysis report
    python -m harness.analyze_run3 --self-check       # arithmetic checks, no data needed
...
def build_report(results: Path) -> str:
    ...
    w("")
    w("Generated by `python -m harness.analyze_run3` from the committed stage-3 JSONL in")
