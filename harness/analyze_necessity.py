...
`harness.analyze_run3`; `leave_one_model_out`, `heterogeneity`, `cluster_bootstrap_mean_diff`,
`stratified_contrast`, `_pooled_discrete`, `_pooled_continuous` come from `harness.analyze_round5`.
...
function then runs completely unmodified.
    ...
    python -m harness.analyze_necessity --self-check                    # arithmetic only, no files
    python -m harness.analyze_necessity                                 # everything found under
                                                                        # --results-dir
    python -m harness.analyze_necessity --section discrete
    python -m harness.analyze_necessity --section e1-estimate           # GPU-cost projection only
...
# `run_necessity.CONDITION_LABELS`: that module also imports `harness.models`, which this section
...
def build_report(results_dir: Path, sections: list[str]) -> tuple[str, dict]:
    ...
    w("")
    w("Generated by `python -m harness.analyze_necessity` from committed `stage_necessity_*.jsonl`")
    w(f"under `{results_dir}`. No GPU, no network, no generation. Every bootstrap uses seed")
    w(f"`{SEED}` with `{RESAMPLES}` resamples, matching `harness/analyze_run3.py` and")
    w("`harness/analyze_round5.py`; every exact test is the same `math.comb`-based binomial")
