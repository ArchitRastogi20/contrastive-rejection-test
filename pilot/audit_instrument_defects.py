...
the item/rejection/corpus-index objects `pilot.repair` already defines.
    ...
    python -m pilot.audit_instrument_defects                        # figures 3, 4, 5 (no corpus needed)
    python -m pilot.audit_instrument_defects --data-file <dump.jsonl>  # adds figures 1 and 2
    python -m pilot.audit_instrument_defects --data-file <dump.jsonl> --verify-reconstruction

`<dump.jsonl>` is a local JSON/JSONL dump of the `framolfese/2WikiMultihopQA` validation split,
in the shape `pilot.data.load_records_from_file` already reads (the same `--data-file` every
...
def build_report(results: Path, data_file: Path | None, verify: bool) -> str:
    ...
    w("")
    w("Generated by `python -m pilot.audit_instrument_defects`. No GPU, no network. Figures 3, 4")
    w("and 5 are computed from committed `code/results/exp3{a,b,c}` JSONL alone; figures 1 and 2")
    w("additionally rebuild the R1-R4 inserted sentences via `pilot.repair`, which needs a local")
